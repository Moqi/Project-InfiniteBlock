Shader "Custom/HeightFog_2"
{
	Properties 
	{
_FogColor("FogColor", Color) = (0.9058824,0.4627451,0.04313726,1)
_FogHeight("FogHeight", Float) = 0.5
_FogHeightFade("FogHeightFade", Float) = -0.2
...
float4 Multiply3=_FogHeight.xxxx * float4( 0.01,0.01,0.01,0.01 );
float4 Multiply2=Multiply3 * _FogHeight.xxxx;
//float4 Splat1=float4( IN.worldPos.x, IN.worldPos.y,IN.worldPos.z,1.0 ).y;
float4 Splat1=float3( IN.worldPos.x, IN.worldPos.y,1.0 ).y;
float4 Invert0= float4(1.0, 1.0, 1.0, 1.0) - Splat1;
float4 Multiply6=_FogHeightFade.xxxx * float4( 0.01,0.01,0.01,0.01 );
float4 Multiply5=Invert0 * Multiply6;
