...
function OnTriggerStay ( other : Collider ) {							// trigger events for collider on push
	if ( other.tag == "push" || other.gameObject.layer == 12 && ControllerSystem.moveSpeed > speedIdle && !ControllerSystem.isCrouching )	// if collider equals pushing object tag name
	{
...
function OnTriggerExit ( other : Collider ) {							// trigger events for collider on push
	if ( other.tag == "push" || other.gameObject.layer == 12 )			// if collider equals pushing object tag name
	{
